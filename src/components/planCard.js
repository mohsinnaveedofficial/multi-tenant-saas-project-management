...
function PlanCard({ title, price, features, highlight, current }) {
  return (
    <div className="relative font-sans">
      ...
      <div
        className={`rounded-2xl p-6 bg-white dark:bg-gray-800 shadow-xs border h-[440px]
        ${highlight ? "border-blue-500 border-4" : "border-gray-200 dark:border-gray-700"}`}
      >
        <div className="flex justify-center">
          <h1 className="text-lg font-bold text-black dark:text-gray-200 pb-2">{title}</h1>
        </div>

        <div className="flex justify-center gap-1">
          <h1 className="text-3xl text-black dark:text-gray-200 font-bold">${price}</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 pt-3.5">/month</p>
        </div>

       
        <div className="text-black mt-4">
          {features.map((item, index) => (
            <div key={index} className="flex text-sm text-gray-500 dark:text-gray-400 ml-2 my-3">
        ...
        <div
          ...
          ${
            current
              ...
              : "bg-gray-900 text-white border-gray-800 dark:bg-gray-800" 
          }`}
        >
