...
function ReportsStatsPanel({ Icon, title, num, stats, color }) {
  ...
  return (
    <div className="border border-gray-200 rounded-2xl bg-white flex justify-between items-center shadow-sm p-3 ps-6 ">
        <div className={`rounded-lg ${selectedColor.text} p-4 text-lg ${selectedColor.bg}`}>
          <Icon />
        </div>
      <div>
        <h5 className="text-gray-600 ">{title}</h5>
        <h3 className="font-bold text-2xl text-gray-900">{num}</h3>
