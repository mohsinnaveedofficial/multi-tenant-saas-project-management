    ...
    <div className="px-4 sm:px-6">
      <div className="mt-4 flex flex-col md:flex-row gap-4 text-sm">
        <div className="rounded-lg border border-gray-100 bg-gray-50 p-4 flex-1">
          ...
          <h2 className="text-gray-500 mt-2">{roleInProject}</h2>
        </div>

        <div className="rounded-lg border border-gray-100 bg-gray-50 p-4 flex-1">
          <h1 className="font-bold">Progress</h1>
          <h2 className="text-gray-500 mt-2">{progress}%</h2>
        </div>
      ...
      <div className="mt-1 text-sm text-gray-500">{project?.description||"Not Any Description"}</div>

      <div className="mt-3 font-bold text-sm">Team Members</div>
      <div className="flex flex-wrap gap-4 mt-2">
        {project?.assignedUsers?.map((member, index) => (
          <div
            key={index}
          ...
          >
            <div className="flex mt-2 gap-2 items-center">
              ...
              <div>
                <h1 className="text-[13px]">{member.user.name}</h1>
                <h2 className="text-gray-500 text-[12px]">{member.roleInProject}</h2>
              </div>
            </div>
          </div>
        ))}
